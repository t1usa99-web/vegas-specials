...
const DETAIL_MASK = "id,location,rating,priceLevel,photos,reviews,regularOpeningHours,nationalPhoneNumber,websiteUri";
...
async function enrichVenue(v) {
  try {
    ...
    if (!p) return 0;
    const photos = (p.photos||[]).slice(0,6).map((x)=>x.name);
    const reviews = (p.reviews||[]).slice(0,3).map((r)=>({author:r.authorAttribution?.displayName||"Google user", avatar:r.authorAttribution?.photoUri||null, rating:r.rating||null, text:(r.text?.text||r.originalText?.text||"").slice(0,600), time:r.relativePublishingTimeDescription||""}));
    await pool.query(
      `UPDATE venues SET lat=COALESCE($2,lat), lng=COALESCE($3,lng), rating=$4, price_level=$5, photo_ref=$6, hours=$7, phone=$8, website=COALESCE($9,website), google_place_id=COALESCE(google_place_id,$10), photos=$11, reviews=$12 WHERE id=$1`,
      [v.id, p.location?.latitude, p.location?.longitude, p.rating||null, (p.priceLevel in PL?PL[p.priceLevel]:null), p.photos?.[0]?.name||null, p.regularOpeningHours?JSON.stringify(p.regularOpeningHours):null, p.nationalPhoneNumber||null, p.websiteUri||null, p.id||null, JSON.stringify(photos), JSON.stringify(reviews)]);
    return 1;
  } catch { return 0; }
}
async function main() {
