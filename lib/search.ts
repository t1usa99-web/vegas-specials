  ...
  try {
    const venues = p.query(
      `SELECT v.id, v.name, v.neighborhood, v.rating, v.cuisine,
       ...
       WHERE v.merged_into IS NULL AND v.hidden IS NOT TRUE AND (v.name ILIKE $1 OR v.neighborhood ILIKE $1 OR v.cuisine ILIKE $1 OR v.vibe_tags::text ILIKE $1)
    ...
    const deals = p.query(
      `SELECT s.id, s.summary, s.days, s.start_time, s.end_time, s.price, v.id venue_id, v.name venue, v.neighborhood
       ...
       WHERE ${GUARD} AND v.hidden IS NOT TRUE AND (s.summary ILIKE $1 OR v.name ILIKE $1) ORDER BY s.confidence DESC LIMIT 30`, [like]);
